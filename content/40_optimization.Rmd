...
This is section __ of a 6-part series on Gradient Descent. Our objective here is to developing the mathematical intuition behind Gradient Descent optimization. 
...
Let's start with a relatively simple function - the univariate differentiable function. How do we minimize this thing? Assuming that $n=1$, we have $f : \mathbb{R} \to \mathbb{R}$, a univariate real-valued function as shown in `r kfigr::figr(label = "convex", prefix = TRUE, link = TRUE, type="Figure")`.
...
To develop the intuition behind the derivative as a limit, consider the function $f(x)=x^2$ as specified in `r kfigr::figr(label = "derivative_as_limit", prefix = TRUE, link = TRUE, type="Figure")`. Select a value along the x-axis, say $x_0$, then designate the corresponding point on the curve, $P=f(x_0)$. Now, press play to move along the x-axis some distance $\Delta x$ from $x_0$ to a new point $x_0+\Delta x$, then plot the corresponding point $Q=f(x_0+\Delta x)$. Next, draw the secant line $PQ$ connecting the two points. Now for the crucial part. Reduce $\Delta x$ such that the variable point $Q$ approaches the fixed point $P$ by sliding along the curve. 
...
So, the derivative is the slope of the tangent line at a point $x_0$.
