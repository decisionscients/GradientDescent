...
In this section, we unveal the underlying mathematical concepts of gradient descent in order to really understand how gradient descent works.

With that, let's start with the relatively trivial task of minimizing a linear function. 

## How to minimize a linear function?
Consider the linear function of the form:
$$f(x)=\theta^Tx+b$$
where     

* $x \in \mathbb{R}^n$ is an $n$-vector,    
* $\theta \in \mathbb{R}^n$ is an $n$-vector, and   
* $b \in \mathbb{R}$ is a scalar.

```{python linear, results='hide', code=readLines('./code/linear.py')}
```

For illustration purposes, the following plot of $f(x)$ assumes $n=1$. 
![](/figures/linear.jpg)
...
## How do we minimizing a univariate differentiable Function?
### Differentiable Functions
First, let's clarify what we mean by *differentiable* function. `r kfigr::figr(label = "differentiability", prefix = TRUE, link = TRUE, type="Figure")` illustrates both differentiable and non-differentiable functions.

![](./figures/differentiability.jpg)

`r kfigr::figr(label = "differentiability", prefix = TRUE, link = TRUE, type="Figure")`: Differentiability

According to our wikipedia friends, "a univariate differentiable function is one whose derivative exists at each point in its domain. We'll clarify what we mean by a derivative in a minute, but this simply means that the graph of a differentiable function must have a (non-vertical) tangent line at each point in its domain, be relatively smooth, and cannot contain any breaks, bends, or cusps " [@wiki:Differentiable]. 
