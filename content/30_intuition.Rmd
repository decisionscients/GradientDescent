...
Let's start with a relatively simple function - the univariate differentiable function. How do we minimize this thing? Let's first suppose that $n=1$, so that $f : \mathbb{R} \to \mathbb{R}$ is a univariate real-valued function as shown in `r kfigr::figr(label = "convex", prefix = TRUE, link = TRUE, type="Figure")`.
...
To be more precise, the basic algorithm for minimizing a univariate ($f(x)\in\mathbb{R}$) differentiable function uses the derivative, $f^\prime(x)$, to determine the direction to move, and is given by:
...
Not all objective functions are as agreeable as the function in `r kfigr::figr(label = "convex_univariate", prefix = TRUE, link = TRUE, type="Figure")`. Consider `r kfigr::figr(label = "nonconvex_univariate", prefix = TRUE, link = TRUE, type="Figure")`. If we start at $x_0$, we end up in the left basin, which happens to be the global minimum; however, If we start at $x_1$, we eventually arrive at a sub-optimal *local* minimum. 
...
The takeaway? Gradient descent is guaranteed to converge to a global minimum with convex objective functions. Since non-convex functions have multiple minima, there is no guarantee that gradient descent will converge to the global minimum. 

## Minimize Multivariate Linear Function
Ok, we've derived the basic gradient descent algorithm for the univariate case ($n=1$). Let's now consider the linear function of the form:
$$f(x)=a^Tx+b$$
where:     
...
We are currently at  $x^T=[0,0]$ and you wish to take a step of one unit of Euclidean distance in the direction that minimizes $f(x)$ the most. Adjust the slider to select a direction in degrees for a unit vector $\vec{u}$ and observe the value of $f(\vec{x}+\vec{u})$. Which direction minimizes $f(x)$ the most? When you find it, note the coordinates of $\vec{u}$.
...
\end{align}
$$
where $\theta$ denotes the angle between the vectors $a$ and $u$. To minimize the function, we must make $\text{cos}\space\theta$ as small as possible (i.e.-1). We do this by choosing $u$ to point in the opposite direction of $a$, the direction of steepest ascent, such that $u=\frac{-a}{\|a\|}$. 
...
This may sound glib, especially given the crazy forms differentiable functions can take. But calculus... uhh...uhh... finds a way! It turns out that any function differentiable at a point can be locally approximated at that point by a linear function. Once we have a local linear approximation of our function, we can use properties of differentiation to optimize the true function $f$. 
...
### The Derivative
A function $f$ is said to be differentiable at $x_0$ if the following limit exists:
$$\displaystyle{\lim_{\Delta x \to0}}\Big[\frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}\Big].$$
...
To develop the intuition behind the derivative as a limit, consider the function $f(x)=x^2$ as specified in `r kfigr::figr(label = "derivative_as_limit", prefix = TRUE, link = TRUE, type="Figure")`. Select a value along the x-axis, say $x_0$, then designate the corresponding point on the curve, $P=f(x_0)$. Now, move along the x-axis some distance $\Delta x$ from $x_0$ to a new point $x_0+\Delta x$, then plot the corresponding point $Q=f(x_0+\Delta x)$. Next, draw the secant line $PQ$ connecting the two points. Now for the crucial part. Reduce $\Delta x$ such that the variable point $Q$ approaches the fixed point $P$ by sliding along the curve. 

What do we notice? First, the secant changes direction as $Q$ approaches point $P$. As the absolute value of $\Delta x$ decreases, the value of $x_0+\Delta x$ approaches $x_0$, and the value of $f(x_0+\Delta x)$ approaches $f(x_0)$. It should be intuitively clear that the tangent line is the limit approached by the secant as $\Delta x\to0$. The slope of the secant line connecting the points ($x_0+\Delta x,f(x_0+\Delta x)$) and ($x_0,f(x_0)$) is the difference in the $y$ values over the difference between the $x$ values. This **difference quotient** becomes:
$$m=\frac{\Delta y}{\Delta x}=\frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}$$
Hence small values of $\Delta x$ that approach zero render secant lines that are increasingly better approximations of the tangent line to $f(x)$ at $x_0$. Since the limiting position as $\Delta x \to 0$ of the secant line is the tangent line, the difference quotient as $\Delta x \to 0$, if it exists, will be the slope of the tangent line at ($x_0, f(x_0)$). This limit is defined as the *derivative* of the function $f$ at $x_0$:
...
$$m=\frac{\Delta y}{\Delta x}=\frac{y-y_0}{x-x_0}=\frac{f(x_0+\Delta x)-f(x_0)}{(x_0+\Delta x)-x_0}=\frac{f(x_0+\Delta x)-f(x_0)}{\Delta x},$$
where $\Delta x=(x-x_0)$.

Using derivatives, we've shown that the slope $m$ approaches the slope of the tangent line $f^\prime(x_0)$ as $\Delta x\to0$. Substituting $f^\prime(x_0)$ for $m$ and rearranging into point slope form yields:
...
Recall that calculus promises that, for a function $f(x_0)$ and a point suitably close to $x_0$, one can pretend that the true function $f$ is just a linear function at $f(x_0)$. This leads us to the most salient interpretation of the tangent line, and it is this.

> The tangent line of a univariate, differentiable function $f$ at point $x_0$ is the best local linear approximation of $f$ at $x_0$. 

If we were to zoom into the graph of a function $f$ at a point $x_0$ using a high-powered microscope (or look at it over a small enough interval), the curve of the function would appear to become increasingly straight. In fact, if we observe $f$ at the intersection of the tangent line, the curve and the tangent line would become qualitatively indistinguishable for all points *near* the point of intersection. 

In fact, the tangent is the *unique* linear function that has the same function value $f(x)$ and derivative at point $x_0$, in this sense it is **best linear approximation** to $f$ at $x_0$. (See [@Bivens1986] for a formal proof.) 
