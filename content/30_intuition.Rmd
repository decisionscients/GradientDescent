...
The unit vector $\vec{u}$ that minimizes $f(x)$ the most is `r [py$u[0], py$u[1]]` and the minimum value at $f(\vec{x}+\vec{u})$ is `r py$z`.  How close did you get?  Turns out the optimum direction is about 31 degrees. 
...
```{python derivative, results='hide', code=readLines('../content/code/derivative.py')}
```
